...
1. Parse the file to extract all snp positions for each individual and add them to set.

2. get union of all snp sets (of size N)

3. for each individual: for each snp in union of snps: assign 0/1 if snp is archaic in individual. This produces a vector of length N.

4. Merge vectors into a matrix you can do PCA on.

5. They already know from the PCA exercise how to label dots in the PCA plot with population and region.

6. Cut out the EPAS1 region from Laurits file and redo the analysis. (This gene maps to 46,520,806-46,613,842 in GRCh37 coordinates).

7. Sum length of admixture segments for each individual.

8. Sum length of admixture segments for each individual in EPAS1 region.

9. Make dots in the PCA plots reflect admixture proportion. When looking at the EPAS1 region they should see Tibetans cluster with as some big dots.
