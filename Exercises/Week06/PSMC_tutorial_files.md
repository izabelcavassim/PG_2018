...
This takes as input an aligned bam file and a reference genome, generates an mpileup using `samtools`, calls the consensus sequence with `bcftools`, and then filters and converts the consensus to FASTQ format. Some parameter explanations:
...
This produces a FASTQ sequence file of chromosome two, but it takes a long time to run (about XX mins) so if you get tired of waiting you can get it here:

    /home/Data/consensus_files_for_pg2018/ERR1025630_sort_dedup_consensus.fq

Here you can also find FASTQ files for all the other individuals  we have been working with.
...
This transforms the consensus sequence into a fasta-like format where the i-th character in the output sequence indicates whether there is at least one heterozygote in the bin [100i, 100i+100).
...
This PSMC analysis takes about 25 minutes to complete. So once you get it started we will talk about the Li et al. paper.
...
Does the plot resemble the ones in Li et al.? We used a different (more correct) mutation rate than Li et al.. What do you think that does to the plot?
